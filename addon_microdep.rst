...
The add-on may be install on different perfSONAR system architectures. Two variants are described in the following subsection.
...
The most straight forward install of *Microdep* is done on a perfSONAR toolkit host (see :doc:`install_quick_start`), i.e. on a host running a full suit of perfSONAR tools.

To add *Microdep* to a toolkit host run::
...
In operational large scale perfSONAR installations system components are typically distributed among several hosts (physical or virtual). One such "hyper distributed" architecture may include 
  ...
  * A **Measurement archive** host (or cluster) running storage components (see :doc:`multi_ma_install`)
  * A **Test point** host, or normally multiple, being hosts running the actual measurements and repoting (raw) results.
...
Note that an empty plot will appear here if your perfSONAR system has no tasks configured, i.e. your perfSONAR archive has never received results from any tests (or your user interface host cannot access you archive host).
...
Note that when installing ``perfsonar-microdep-toolkit`` on a toolkit host the default configuration files in the add-on packages should ensure operation without additional configuration.
...
*Microdep* analyses raw results from **latencybg tests**, and results from **traceroute tests** (both UDP and TCP based). The following subsections present some details of the analysis performed and the new (aggregated) results generated.

Gap analysis
^^^^^^^^^^^^

Microdep search for *Gaps* in data flows from latencybg tests (run by owamp tools) and generate event records when gaps are found. Event records are pushed to a **microdep_gap_ana** Opensearch index on the *Archive* host.
...
Microdep calculates *Jitter* in data flows from latencybg test (run owamp tools) by following reccommendations from `RFC3550 <https://datatracker.ietf.org/doc/html/rfc3550#section-6.4.1>`_.
...
where *D(i,j)* is difference in packet spacing with *Si* and *Ri* being timestamps of a packet when sent and received respectively, and *J(i)* is jitter after receiving packet number *i*. *W* is a weighting factor set to 16 in RFC3550 but 5 (``rtp:``) in *Microdep*.

Jitter event records (with event_type = "jitter") are pushed to the same index as gap events , i.e. the **microdep_gap_ana** Opensearch index on the *Archive* host.  The maximum time between a jitter event records is 600 seconds (``jitter:``).
...
As an supplement to jitter measurements Microdep attempts to discover queuing events by looking for radical changes in average end-to-end delay of last 50 (``win:``) packets seen and the minimum end-to-end delay observed in a limited recent time window of 10000 (``slep:``) packets. This  delay difference is termed **Queuing time**. Two thresholds apply:

  * When Queuing time exceeds 10ms (``ddelay_high:``) significant queuing is considered to take place (i.e. a *queuing event* has started), and an extra jitter event record (see :ref:`addon_microdep_jitter-analysis`) is output which include a queuing time value (h_ddelay).
  * When Queuing time returns below 2ms (``ddelay_low:``) queuing is no longer considered significant (i.e. a *queuing event* has concluded). An extra jitter event record is again output.

The more relevant data for queuing analysis output in jitter event records are:
...
Microdep searchs for and reports *Route errors* based on output from traceroute tests, both UDP and TCP. A simple anomaly detection algorithm is applied to enable event records output only when the status in a traceroute data set changes significantly.
...
Route errors are documented by four type of event records:
  * **Route warning** (event_type = "routewarn"): An abnormal traceroute is observed, being the (potential) start of a sequence of abnormal traceroutes.
  * **Route error** (event_type = "routeerr"): An number of abnormal traceroutes (default is 1, ``f_csensitivity:``) has being observed.
  * **Route normal** (event_type = "routenorm"): The most common traceroute observed in a window of 2000 traceroutes (2 x ``f_max_state_counter:``) has changed and is now considered the new normal. Such a traceroute must have 30 more occurrences (``f_majority:``) than the second most common traceroute observed.  
  * **Route summary** (event_type = "routesum"): The *perfsonar-microdep-trace-ana* service has been stopped/restarted, triggering the output of a summary event record for the time period it has been running. Default reset interval is 24h.   

Route change analysis
^^^^^^^^^^^^^^^^^^^^^

Microdep searchs for and reports *Route changes* based on output from traceroute tests, both UDP and TCP. A cross-entropy based algorithm is applied to detect when a route has changed significantly.
...
The algorithm applied has in short the following steps:
  * When a new traceroute measurement is available for a flow, a database of counters is updated, counting occurrences of specific IP-address at specific hops in the route.
...
In the data set reported by *Route change* event some of the more relevant values are
  ...
  * **routechange_ip**: IP-address in hop before first hop with high delta-ce value, i.e. the IP-address of the router potentially responsible for a route change.
...
**Route summary** output when the *perfsonar-microdep-trace-ana* service has been stopped/restarted also include summaries of route change properties.
...
A running measurement setup, with traceroute and latencybg test (the latter with *output-raw* enabled, see :ref:`addon_microdep_configuration`) results in a main map GUI similar to the image below

.. image:: images/addon_microdep_main-map-gui.png
        :target: _images/addon_microdep_main-map-gui.png
        :scale: 40 %
...
When Microdep's analytic services find new traceroute or latencybg test results in the archive, a number event records may be generated (see :ref:`addon_microdep_operation`). The *world map* in the web user interface uses *topology event* records to present the test flow topology being analyzed. The image below shows such an example topology. 
...
Testpoint hosts are place at their geo-coordinates (when available), and grey lines are drawn between the testpoints running measurements (left image above). The lines stay grey until at least one other (non-topology) event becomes available.

*Traffic light rating* (TLR) is applied to flow lines when results are available. Lines are colored by green, yellow or red according to the significance of the events reported (right image above). The TLR coloring reflects values for the currently selected *property* in the navigation/search bar. A legend shows what value range results in which color.
...
On mouse-over of a flow line a small window pops up with basic info about the flow. When a flow line is clicked a larger and more comprehensive window pops up titled "Link details". See image below. 
...
The link details windows presents
  ...
  * A button for opening a traceroute viewer for the flow in a seperate tab (next to the map). See :ref:`addon_microdep_traceroute-viewer`.
...
Some alternative views are available for presenting the core data set (which feeds the map and link detail popup window). By clicking *+* in the tab contol area in the upper left corner of the map (see image below), a menu appears offering to open new tabs

.. image:: images/addon_microdep_map-gui-tab-menu.png
        :target: _images/addon_microdep_map-gui-tab-menu.png
        :scale: 40 %

Available tabs are:
  * **Heatmap**: Presents a table view variant of the flow topology including values for selected property.
