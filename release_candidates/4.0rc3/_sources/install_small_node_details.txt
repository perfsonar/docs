...
#. None of these devices are powerful enough to run the full perfSONAR toolkit which installs a measurement archive.
#. pScheduler may not be suitable for use on pi-class/ARM devices.
...
More stable are the small nodes closer in cost to $200, such as the Intel NUC series, Zotac Zbox ci320 (2.1GHz CPU)/ci323 (dual NIC), GIGABYTE BRIX and various other examples available on the market as barebones small PC solutions. They may be capable of running perfSONAR Toolkit with a small test mesh.
...
The :doc:`perfsonar-testpoint <install_options>` bundle can be used to install everything you need to add a low-end node to a centrally managed test mesh with a central measurement archive. See instructions for installing the :doc:`perfsonar-testpoint <install_options>` bundle: 
:doc:`install_centos`,
:doc:`install_debian`
